...
/// </summary>
VECTOR StageCollision::CheckCollision(Character& object, const VECTOR& moveVector)
{
...
/// </summary>
VECTOR StageCollision::CheckHitWithWall(Character& object, const VECTOR& checkPosition)
{
    VECTOR fixedPos = checkPosition;

    float HitRadius = object.GetHitRadius();
    float HitHeight = object.GetHitRadius();
...
/// </summary>
VECTOR StageCollision::CheckHitWithFloor(Character& object, const VECTOR& checkPosition)
{
    VECTOR fixedPos = checkPosition;
    ...
    // ジャンプ中且つ上昇中の場合は処理を分岐
    if (state == State::Jump && jumpPower > 0.0f) // Jumping upward
    {
        // 天井に頭をぶつける処理を行う
